...
try {
  ...
  const blind = await functies.voerUit('kavelKiezen', { perceelId: '22720386570000' })
  ...
  const adapter = maakTestAdapter({
    script: [
      [
        { functie: { naam: 'kavelZoeken', args: { adres: 'Tweetandschelp 52, Monster' } } },
        { zeg: 'Ik heb Tweetandschelp 52 in Monster gevonden. Wijs op de kaart uw perceel aan, of noem het perceelnummer.' },
      ],
      [
        { functie: { naam: 'kavelKiezen', args: { perceelId: '22720386570000' } } },
        { zeg: 'Vastgelegd: perceel I 3865, 260 vierkante meter volgens het Kadaster. Hoeveel woonoppervlakte wenst u ongeveer?' },
      ],
      [
  ...
  await adapter.start()
  await adapter.zegTekst('het adres is Tweetandschelp 52 in Monster')
  await adapter.zegTekst('mijn perceel is nummer 3865')
  ...
  eis('kavel vastgelegd met adres en perceel',
    eind.kavel?.perceelId === '22720386570000' && eind.kavel?.adres?.includes('Tweetandschelp 52'))
  eis('oppervlakte komt uit de kadastrale data (260 m2)', eind.kavel?.oppervlakte === 260)
  ...
  const zoek2 = await f2.voerUit('kavelZoeken', { adres: 'Tweetandschelp 52, Monster' })
  eis('kavelZoeken bevestigt pas ok met geladen percelen (aantalPercelen)',
    zoek2.ok === true && zoek2.aantalPercelen === 12)
  await f2.voerUit('kavelKiezen', { perceelId: '22720513070000' })
  ...
  eis('tweede perceel heeft eigen kadastrale oppervlakte (3395 m2)', ref2.huidige.kavel?.oppervlakte === 3395)
} finally {
