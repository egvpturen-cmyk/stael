...
// drie favorieten kiezen via de beelden; per klik wachten tot de
// teller meebeweegt, want de sessie-patch loopt via de echte API
let gekozen = 0
for (const n of [1, 3, 25]) {
  await page.click('.collectiekaart:nth-of-type(' + n + ') img')
  gekozen++
  await page.waitForFunction(t => document.body.textContent.includes('(' + t + ' gekozen)'), gekozen, { timeout: 15000 })
}
console.log('favorietenteller staat op 3: ok')
