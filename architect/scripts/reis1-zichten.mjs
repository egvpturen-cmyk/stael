...
if (overflow > 0) console.log('breedste elementen:', await mob.evaluate(() => [...document.querySelectorAll('*')].filter(e => e.getBoundingClientRect().right > window.innerWidth + 1).slice(0, 5).map(e => (e.className || e.tagName) + '>' + Math.round(e.getBoundingClientRect().right)).join(' | ')))
